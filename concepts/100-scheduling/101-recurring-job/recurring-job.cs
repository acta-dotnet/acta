...
namespace Acta.Concepts.Recurring
{
    ...
    public sealed class PurgeStaleSessionsJob
    {
        ...
        public async Task<int> Handle(PurgeStaleSessions input, JobContext ctx, CancellationToken ct)
        {
            await Task.Delay(100, ct);
            var purged = Random.Shared.Next(0, 25);
            Console.WriteLine($"{DateTime.Now:HH:mm:ss} [{string.Join(", ", ctx.TriggeringScheduleNames)}] purged {purged} stale sessions");
            return purged;
        }
    }
}
