...
namespace Acta.Concepts.ResetState
{
    ...
    public sealed class MetricsMonitor
    {
        ...
        public async Task Handle(CollectMetrics input, JobContext ctx, CancellationToken ct)
        {
            // Reads 0 because the previous cycle's reset wiped this variable; without it, reads 3.
            var carried = await ctx.GetVariableOrDefaultAsync("reading-count", 0, ct);
            ...
            for (var i = 1; i <= 3; i++)
            {
                ...
                sum += reading;
                await ctx.SetVariableAsync("reading-count", i, ct);
