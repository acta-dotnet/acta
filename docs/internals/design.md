...
- **The schema baseline (M001) is frozen.** Every schema change is an additive `Mnnn` migration (new columns/tables): no renames, drops, retypes, or renumbered code values. *Reason:* closing the public, physical, and numeric vocabulary together before upgrade compatibility is promised.
