...
- **Allowed dependency graph:** Execution depends only on shared kernel primitives; Alerting and Outbox depend on Execution's API; Operations reads Execution, Alerting, and Outbox APIs (the one sanctioned cross-module read surface); hosting/composition sees everything. No module depends on Operations, and no module touches another module's stores or tables for writes. Ports invert at the seams: Execution declares `IAlertSink` and `IJobSubmission`; Alerting and Outbox implement or consume them. One declared exception remains: worker registration carries alerting channel declarations from the builder to the channel registry. *Reason:* recording the target graph makes each extraction PR checkable against a decision instead of taste.
