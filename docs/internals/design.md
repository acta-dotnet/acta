...
- **Allowed dependency graph:** each module exposes an `Api` sub-namespace (its declared contract) and hides everything else. The gated edges are: Alerting and Outbox -> `Execution.Api` (`IJobSubmission`, `IAlertSink`, the control contract); Execution -> `Alerting.Api` (channel declarations and registry, consumed at worker startup) and -> `Operations.Api` (the event feed backing `IJobs.ListJobEventsAsync`). Operations is the one sanctioned unrestricted cross-module reader. Hosting/composition sees everything, and `WorkerRegistration` is a hosting type for exactly that reason. No module writes another module's tables, and no module takes or service-locates another module's store ports. *Reason:* recording the target graph makes each extraction PR checkable against a decision instead of taste; the reference gate holds the graph to zero undeclared edges.
