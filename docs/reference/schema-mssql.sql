...
-- schema name 'acta' throughout. Baseline stamp: baseline-2978ad1ccb8de123488c4581477c134d.
--
...
CREATE INDEX ix_runtimes_retention ON acta.runtimes (namespace_id, retention_until_utc, job_id) WHERE retention_until_utc IS NOT NULL AND status_code IN (100, 200, 220) WITH (OPTIMIZE_FOR_SEQUENTIAL_KEY = ON);
CREATE INDEX ix_runtimes_worker_inflight ON acta.runtimes (leased_by_worker_id, job_id) WHERE leased_by_worker_id IS NOT NULL AND status_code IN (40, 50);
END
GO
...
VALUES (0, 'baseline-2978ad1ccb8de123488c4581477c134d', 'acta');
...
END;
GO
CREATE OR ALTER PROCEDURE acta.resolve_job_alerts
    @p_namespace_id INT,
    @p_job_id BIGINT,
    @p_source_event_id BIGINT
AS
...
END;
GO
CREATE OR ALTER PROCEDURE acta.update_alert_delivery
    @p_id BIGINT,
    @p_version INT,
    @p_delivery_status_code TINYINT,
...
CREATE OR ALTER PROCEDURE acta.repair_recovery_slot
    @p_namespace_id INT,
    @p_job_id BIGINT
AS
BEGIN
    ...
    BEGIN TRY
        ...
        FROM @repaired x
        INNER JOIN acta.jobs j ON j.id = x.job_id;
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @tenant_id = id
        FROM acta.tenants WITH (UPDLOCK, HOLDLOCK)
...
END;
GO
CREATE OR ALTER PROCEDURE acta.acquire_slot
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM acta.locks WHERE lock_key = @reservation)
            BEGIN
                SELECT @stored = l.expires_at_utc
                FROM acta.locks AS l WITH (UPDLOCK, HOLDLOCK)
        ...
        -- the turn is now.
        SELECT
            COALESCE((SELECT r.expires_at_utc FROM acta.locks r WHERE r.lock_key = @reservation), @now) AS resume_at_utc,
            CASE WHEN EXISTS (SELECT 1 FROM acta.locks r WHERE r.lock_key = @reservation) THEN 0 ELSE 1 END AS admitted;
