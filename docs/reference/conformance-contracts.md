...
- **Guarantees:**
  - A one-shot that throws until its budget is gone writes one finished event and opens a FinalFailure
  - A success at the failures-only audit level writes no finished event and opens no alert
- **Store methods:**
  - `Acta.Runtime.Modules.Alerting.IAlertStore.GetAlertableEventsAsync`
