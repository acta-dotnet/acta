...
-- Lease
IF OBJECT_ID(N'acta.leases', N'U') IS NULL
BEGIN
CREATE TABLE acta.leases (
...
CREATE INDEX ix_leases_reclaim_expired ON acta.leases (kind_code, expires_at_utc);
END
GO
...
CREATE INDEX ix_workers_namespace_status_lastseen ON acta.workers (namespace_id, status_code, last_seen_at_utc);
CREATE INDEX ix_workers_status_lastseen ON acta.workers (status_code, last_seen_at_utc);
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @definition_id = j.definition_id, @lineage_root_id = j.lineage_root_id
          FROM acta.jobs j WHERE j.id = @job_id;
        SELECT @execution_number = r.execution_number
          FROM acta.runtimes r WHERE r.job_id = @job_id;
...
BEGIN
    ...
    DECLARE @v_ns SMALLINT = (SELECT id FROM acta.namespaces WHERE name = @p_namespace_name);
    IF @v_ns IS NULL
        THROW 50000, 'raise_job_alert: unknown namespace', 1;

    DECLARE @v_job_ref UNIQUEIDENTIFIER = (SELECT job_ref FROM acta.jobs WHERE id = @p_job_id);
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            END;
            IF EXISTS (SELECT 1 FROM @p_batch b JOIN acta.tenants t ON t.tenant_key = b.tenant_key
...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT @schedule_ids SELECT id FROM acta.schedules WITH (UPDLOCK, ROWLOCK) WHERE job_id = @p_id;
        INSERT @alert_ids SELECT id FROM acta.alerts WITH (UPDLOCK, ROWLOCK) WHERE job_id = @p_id;
        INSERT @event_ids SELECT id FROM acta.events WITH (UPDLOCK, ROWLOCK) WHERE job_id = @p_id;
    ...
    BEGIN TRY
        ...
        SELECT
          ...
          INNER JOIN acta.jobs AS j
                 ...
                 AND j.parent_id IS NULL
          INNER JOIN acta.definitions AS jd ON jd.id = d.definition_id
         WHERE NOT EXISTS (SELECT 1 FROM acta.runtimes AS r WHERE r.job_id = j.id);
...
BEGIN
    ...
    BEGIN
        SELECT @namespace_id = n.id FROM acta.namespaces n WHERE n.name = @p_namespace_name;
        IF @namespace_id IS NULL
        BEGIN
            SELECT CAST(2 /* AdminControlAction.NotFound */ AS SMALLINT) AS action, CAST(NULL AS INT) AS version;
            RETURN;
        ...
        BEGIN
            SELECT @definition_id = d.id
              FROM acta.definitions d WHERE d.namespace_id = @namespace_id AND d.name = @p_job_name;
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @p_tenant_key, @p_display_name, @p_description, 10 /* TenantStatusCode.Active */, @now, @now, 0
         WHERE NOT EXISTS (SELECT 1 FROM acta.tenants WHERE tenant_key = @p_tenant_key);
    END TRY
    BEGIN CATCH
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @p_name, @p_owner_team, @p_description, @p_catalog_hash, @p_status_code, @now, @now, 0
         WHERE NOT EXISTS (SELECT 1 FROM acta.namespaces WHERE name = @p_name);

        SELECT @ns_id = id FROM acta.namespaces WHERE name = @p_name;
...
BEGIN
    ...
    BEGIN
        ...
        DELETE FROM acta.checkpoints WHERE job_id IN (SELECT id FROM @del);
        DELETE FROM acta.runtimes    WHERE job_id IN (SELECT id FROM @del);
        DELETE FROM acta.steps       WHERE job_id IN (SELECT id FROM @del);
        DELETE FROM acta.results     WHERE job_id IN (SELECT id FROM @del);
        DELETE FROM acta.jobs WHERE id IN (SELECT id FROM @del);
    ...
    BEGIN
        ...
        DELETE FROM acta.tags WHERE scope_code = 90 /* TagScopeCode.Event */ AND scope_id IN (SELECT id FROM @del);
    ...
    BEGIN
        ...
        INSERT INTO @del(id)
            SELECT TOP (@p_batch_size) id
             ...
             ORDER BY created_at_utc, id;
        DELETE FROM acta.tags WHERE scope_code = 80 /* TagScopeCode.Alert */ AND scope_id IN (SELECT id FROM @del);
    ...
    BEGIN
        ...
        DELETE FROM acta.tags WHERE scope_code = 70 /* TagScopeCode.Worker */ AND scope_id IN (SELECT id FROM @del);
    ...
    BEGIN
        ...
        SELECT TOP (@p_batch_size) lease_key
          FROM acta.leases WITH (UPDLOCK, READPAST)
         WHERE kind_code = 10 /* LeaseKindCode.Lock */
           AND expires_at_utc <= @now
         ORDER BY expires_at_utc;
        DELETE t FROM acta.leases t INNER JOIN @lock_del d ON d.lease_key = t.lease_key;
        SET @rows = @@ROWCOUNT;
...
BEGIN
    ...
    IF @@ROWCOUNT = 0 AND NOT EXISTS (SELECT 1 FROM acta.leases WHERE lease_key = @p_lease_key)
    BEGIN
        BEGIN TRY
