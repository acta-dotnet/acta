...
-- schema name 'acta' throughout. Baseline stamp: init-extensible-status-v1.
--
...
-- Schema placeholder `acta` is substituted by the migration runner at apply time.
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_alerts_dedupe ON acta.alerts (namespace_id, dedupe_key, dedupe_window_start_utc) WHERE dedupe_key IS NOT NULL;
...
-- Lease
CREATE TABLE IF NOT EXISTS acta.leases (
...
CREATE INDEX IF NOT EXISTS ix_leases_reclaim_expired ON acta.leases (kind_code, expires_at_utc);
...
CREATE INDEX IF NOT EXISTS ix_workers_namespace_status_lastseen ON acta.workers (namespace_id, status_code, last_seen_at_utc);
CREATE INDEX IF NOT EXISTS ix_workers_status_lastseen ON acta.workers (status_code, last_seen_at_utc);
...
LEFT JOIN acta.results AS lr
  ON lr.job_id = j.id
 AND lr.execution_number = (SELECT MAX(rr.execution_number) FROM acta.results AS rr WHERE rr.job_id = j.id);
...
SELECT
    ...
    CASE s.misfire_strategy_code WHEN 10 THEN 'fire-once-catch-up' WHEN 20 THEN 'skip' END AS misfire_strategy,
...
BEGIN
    ...
    SELECT j.definition_id, j.lineage_root_id INTO v_definition_id, v_lineage_root_id
      FROM acta.jobs j WHERE j.id = v_job_id;
    SELECT r.execution_number INTO v_execution_number
      FROM acta.runtimes r WHERE r.job_id = v_job_id;
            ...
            DELETE FROM acta.results r
             WHERE r.job_id = p_id
               AND r.execution_number NOT IN (
                   SELECT execution_number FROM acta.results
...
BEGIN
    ...
    upserted AS (
        ...
        ON CONFLICT (namespace_id, name) DO UPDATE SET
          ...
          WHERE EXCLUDED.manifest_generation_at_utc >= acta.definitions.manifest_generation_at_utc
            AND (
...
BEGIN
    ...
    SELECT jd.retention_seconds_effective INTO v_retention_seconds
      FROM acta.definitions jd WHERE jd.id = v_definition_id;
...
BEGIN
    SELECT n.id, n.status_code, n.version INTO v_id, v_status, v_version
      FROM acta.namespaces n WHERE n.name = p_namespace_name FOR UPDATE;
    IF NOT FOUND THEN
...
BEGIN
    ...
    ON CONFLICT (job_id, name) DO UPDATE SET
        ...
        status_code                   = CASE WHEN acta.schedules.status_code = 230 /* ScheduleStatusCode.Orphaned */
                                             ...
                                             ELSE acta.schedules.status_code END,
        description                   = EXCLUDED.description,
        modified_at_utc               = now(),
    ...
    IF p_namespace_name IS NOT NULL THEN
        ...
        ELSE
            SELECT d.id INTO v_definition_id
              FROM acta.definitions d WHERE d.namespace_id = v_namespace_id AND d.name = p_job_name;
            IF NOT FOUND THEN
                RETURN QUERY SELECT 2 /* AdminControlAction.NotFound */::SMALLINT, NULL::INT; RETURN;
            END IF;
...
BEGIN
    SELECT t.id, t.version INTO v_id, v_version
      FROM acta.tenants t WHERE t.tenant_key = p_tenant_key FOR UPDATE;
...
BEGIN
    ...
    ELSE
        INSERT INTO acta.checkpoints (job_id, kind_code, name, status_code, due_at_utc, created_at_utc, modified_at_utc, version)
...
BEGIN
    ...
    WHILE v_rows > 0 AND v_iter < p_max_iterations LOOP
        ...
        v_rows := COALESCE(cardinality(v_ids), 0);
        IF v_rows > 0 THEN
            DELETE FROM acta.tags WHERE scope_code = 90 /* TagScopeCode.Event */ AND scope_id = ANY(v_ids);
    ...
    WHILE v_rows > 0 AND v_iter < p_max_iterations LOOP
        ...
        v_rows := COALESCE(cardinality(v_ids), 0);
        IF v_rows > 0 THEN
            DELETE FROM acta.tags WHERE scope_code = 80 /* TagScopeCode.Alert */ AND scope_id = ANY(v_ids);
    ...
    WHILE v_rows > 0 AND v_iter < p_max_iterations LOOP
        ...
        v_rows := COALESCE(cardinality(v_ids), 0);
        IF v_rows > 0 THEN
            DELETE FROM acta.tags WHERE scope_code = 70 /* TagScopeCode.Worker */ AND scope_id = ANY(v_ids);
    ...
    WHILE v_rows > 0 AND v_iter < p_max_iterations LOOP
        ...
        SELECT array_agg(q.lease_key) INTO v_lease_keys FROM (
            SELECT lease_key FROM acta.leases
             WHERE kind_code = 10 /* LeaseKindCode.Lock */
               AND expires_at_utc <= v_now
             ORDER BY expires_at_utc
             LIMIT p_batch_size
             FOR UPDATE SKIP LOCKED) q;
        DELETE FROM acta.leases WHERE lease_key = ANY(v_lease_keys);
        GET DIAGNOSTICS v_rows = ROW_COUNT;
...
LANGUAGE sql
AS $$
    INSERT INTO acta.leases (lease_key, kind_code, job_id, expires_at_utc, version)
