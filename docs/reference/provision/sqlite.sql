...
-- schema name 'main' throughout. Baseline stamp: init-extensible-status-v1.
--
...
-- Schema placeholder `main` is substituted by the migration runner at apply time.
-- Routine and view bodies are not baked in: the SQL object installer applies them after migrations.
-- SQLite: no schema container (main is the attached database, normally `main`);
...
-- SQLite has no schema container; tables are created in the attached database (main).
...
-- Lease
CREATE TABLE IF NOT EXISTS main.leases (
...
CREATE INDEX IF NOT EXISTS main.ix_leases_reclaim_expired ON leases (kind_code, expires_at_utc);
...
CREATE INDEX IF NOT EXISTS main.ix_workers_namespace_status_lastseen ON workers (namespace_id, status_code, last_seen_at_utc);
CREATE INDEX IF NOT EXISTS main.ix_workers_status_lastseen ON workers (status_code, last_seen_at_utc);
...
VALUES (1, 'init-extensible-status-v1', 'main')
...
LEFT JOIN main.results AS lr
  ON lr.job_id = j.id
 AND lr.execution_number = (SELECT MAX(rr.execution_number) FROM main.results AS rr WHERE rr.job_id = j.id);
