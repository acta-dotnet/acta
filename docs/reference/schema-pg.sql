...
-- schema name 'acta' throughout. Baseline stamp: baseline-e1a1f3207caf5a295743ba88d8c3ca26.
--
...
CREATE INDEX IF NOT EXISTS ix_runtimes_claim_ready ON acta.runtimes (namespace_id, priority_code DESC, next_run_at_utc, job_id, status_code) WHERE status_code IN (10, 20);
...
CREATE INDEX IF NOT EXISTS ix_runtimes_worker_inflight ON acta.runtimes (leased_by_worker_id, job_id) WHERE leased_by_worker_id IS NOT NULL AND status_code IN (40, 50);
...
VALUES (0, 'baseline-e1a1f3207caf5a295743ba88d8c3ca26', 'acta')
...
SELECT
    ...
    d.concurrency_limit_effective AS concurrency_limit,
    d.concurrency_limit AS concurrency_limit_default,
    d.concurrency_limit_override,
    d.rate_limit_effective AS rate_limit,
    d.rate_limit AS rate_limit_default,
...
CREATE OR REPLACE FUNCTION acta.resolve_job_alerts(
    p_namespace_id INT,
    p_job_id BIGINT,
    p_source_event_id BIGINT
)
...
CREATE OR REPLACE FUNCTION acta.update_alert_delivery(
    p_id BIGINT,
    p_version INT,
    p_delivery_status_code SMALLINT,
...
CREATE OR REPLACE FUNCTION acta.acquire_slot(
...
BEGIN
    ...
    RETURN QUERY
    SELECT
        COALESCE((SELECT r.expires_at_utc FROM acta.locks r WHERE r.lock_key = v_reservation), v_now),
        NOT EXISTS (SELECT 1 FROM acta.locks r WHERE r.lock_key = v_reservation);
END;
$$;
