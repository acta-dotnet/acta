...
/// <summary>
/// Builder contract for <c>IJobsBuilder.Reference</c>: a reference contributes a catalog (namespace +
...
public sealed class ReferenceRegistrationTests
{
    ...
    {
        var builder = new JobsBuilder(new ServiceCollection());
        ...
        Assert.Equal(typeof(FakeManifest), Assert.Single(catalog.Modules).ManifestType);
    }
