...
public sealed class TagEndpointTests
{
    ...
    public async Task Get_tags_returns_the_target_tag_set_for_every_entity_route()
    {
        ...
        {
            var response = await client.GetAsync($"/acta/jobs/api/{path}", TestContext.Current.CancellationToken);
    ...
    public async Task Get_tags_maps_unknown_target_to_404()
    {
        ...
        var unknown = await client.GetAsync($"/acta/jobs/api/jobs/{Found}/tags", TestContext.Current.CancellationToken);
        var malformedRef = await client.GetAsync("/acta/jobs/api/jobs/42/tags", TestContext.Current.CancellationToken);
        var invalidName = await client.GetAsync("/acta/jobs/api/namespaces/Bad_NS!/tags", TestContext.Current.CancellationToken);
    ...
    public async Task Upsert_tag_records_input_and_returns_applied()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, $"/acta/jobs/api/jobs/{Found}/tags")
        {
    ...
    public async Task Remove_tag_records_name_and_returns_applied()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Delete, $"/acta/jobs/api/jobs/{Found}/tags/env");
        request.Headers.Add(Confirm, "true");
        var response = await client.SendAsync(request, TestContext.Current.CancellationToken);
    ...
    public async Task Tag_mutations_require_the_confirmation_header()
    {
        ...
        var post = await client.PostAsync(
            $"/acta/jobs/api/jobs/{Found}/tags",
            JsonContent.Create(new { name = "env" }),
            TestContext.Current.CancellationToken
        );
        var delete = await client.DeleteAsync($"/acta/jobs/api/jobs/{Found}/tags/env", TestContext.Current.CancellationToken);
    ...
    public async Task Tag_mutations_map_not_found_and_invalid_targets()
    {
        ...
        using var missing = new HttpRequestMessage(HttpMethod.Post, $"/acta/jobs/api/jobs/{Found}/tags")
        {
        ...
        using var invalid = new HttpRequestMessage(HttpMethod.Post, "/acta/jobs/api/namespaces/Bad_NS!/tags")
        {
            Content = JsonContent.Create(new { name = "env" }),
        };
    ...
    public async Task Jobs_list_binds_repeated_tag_filters()
    {
        ...
        var response = await client.GetAsync("/acta/jobs/api/jobs?tag=env:prod&tag=team", TestContext.Current.CancellationToken);
    ...
    public async Task Events_list_binds_repeated_tag_filters()
    {
        ...
        var response = await client.GetAsync("/acta/jobs/api/events?tag=env:prod", TestContext.Current.CancellationToken);
