...
public sealed class DeadlineProbes
{
    ...
    public async Task Strict(JobContext ctx, CancellationToken ct) => await ctx.SetVariableAsync("ran", true, ct);
    ...
    public async Task Advisory(JobContext ctx, CancellationToken ct) => await ctx.SetVariableAsync("overdue", ctx.IsOverdue, ct);
    ...
    public Task Retry(JobContext ctx, CancellationToken ct) => throw new InvalidOperationException("boom");
}
