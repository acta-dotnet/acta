...
BEGIN
    ...
    reclaimed AS (
        UPDATE {{schema}}.runtimes r
         ...
         WHERE r.job_id = s.id
        RETURNING r.job_id AS id, j.namespace_id, r.execution_number, j.lineage_root_id,
    ...
    event_insert AS (
        ...
        SELECT
            41 /* JobEventCode.JobExecutionFinished */, now(), r.namespace_id,
            10 /* JobActorCode.Sys */, NULL,
            r.id, r.job_ref, r.execution_number,
            COALESCE(r.lineage_root_id, r.id), r.definition_id, r.tenant_id,
            NULL,
            r.from_status, r.new_status,
