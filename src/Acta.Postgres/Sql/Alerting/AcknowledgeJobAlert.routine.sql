...
BEGIN
    ...
    SELECT j.definition_id, j.lineage_root_id INTO v_definition_id, v_lineage_root_id
      FROM {{schema}}.jobs j WHERE j.id = v_job_id;
    SELECT r.execution_number INTO v_execution_number
      FROM {{schema}}.runtimes r WHERE r.job_id = v_job_id;

    v_ack := now();

    UPDATE {{schema}}.alerts
       SET acknowledged_at_utc = v_ack,
           modified_at_utc     = now(),
           version             = version + 1
     WHERE id = p_id;
    ...
    VALUES (
        140 /* JobEventCode.AlertAcknowledged */, now(), v_namespace_id,
        p_actor_code, p_actor_key,
        v_job_id, v_job_ref, v_execution_number,
        COALESCE(v_lineage_root_id, v_job_id), v_definition_id,
        NULL,
        NULL, NULL,
        NULL, NULL,
