SELECT
    ...
    d.name AS job_name,
    s.name AS step_name,
    {{decode:job-step-status:s.status_code}} AS state,
    s.status_code,
    s.attempt_number,
    s.next_retry_at_utc,
    ...
    CASE WHEN s.result_format_id IN (1 /* JobPayloadFormat.Json */, 3 /* JobPayloadFormat.Text */) THEN convert_from(s.result, 'UTF8') END AS result_text,
    s.created_at_utc,
    s.modified_at_utc,
    s.version
