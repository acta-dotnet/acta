SELECT s.id, s.job_id, s.definition_id, ns.name, jd.name, s.name,
        ...
        SELECT 1
         ...
         WHERE NOT EXISTS (
               SELECT 1 FROM {{schema}}.tags tg
                WHERE tg.scope_code = 60 /* TagScopeCode.Schedule */ AND tg.scope_id = s.id
                  AND tg.name = f.value->>'name'
                  AND ((f.value->>'value_search') IS NULL OR tg.value_search = f.value->>'value_search')
         )
        ))
      AND (@p_cursor_next_run_at_utc IS NULL
