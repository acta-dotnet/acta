...
AS $$
    WITH ns_upsert AS (
        ...
        VALUES (p_name, p_owner_team, p_description, p_catalog_hash, p_status_code, now(), now(), 0)
    ...
    ns AS (
        ...
        SELECT id FROM {{schema}}.namespaces
         WHERE name = p_name AND NOT EXISTS (SELECT 1 FROM ns_upsert)
    ),
    w AS (
        ...
        SELECT ns.id, 10 /* WorkerStatusCode.Active */, p_deployment_version, p_host, p_engine_version, p_dotnet_version, p_process_id, p_max_concurrency, now(), now(), now(), 0
          FROM ns
    ...
    evt AS (
        ...
        SELECT 120 /* JobEventCode.WorkerStarted */, now(), w.namespace_id, 70 /* JobActorCode.Worker */, w.id::VARCHAR,
