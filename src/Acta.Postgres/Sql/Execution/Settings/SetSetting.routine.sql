...
DECLARE
    ...
    v_namespace_id SMALLINT; v_definition_id INT; v_scope_id INT; v_version INT;
BEGIN
    IF p_namespace_name IS NOT NULL THEN
        ...
        IF p_job_name IS NULL THEN
            v_scope_code := 30 /* SettingScopeCode.Namespace */; v_scope_id := v_namespace_id;
        ELSE
            SELECT d.id INTO v_definition_id
              FROM {{schema}}.definitions d WHERE d.namespace_id = v_namespace_id AND d.name = p_job_name;
            IF NOT FOUND THEN
                RETURN QUERY SELECT 2 /* AdminControlAction.NotFound */::SMALLINT, NULL::INT; RETURN;
            END IF;
            v_scope_code := 40 /* SettingScopeCode.Definition */; v_scope_id := v_definition_id;
        END IF;
    END IF;

    UPDATE {{schema}}.settings s
       SET value_format_id = p_value_format_id, value = p_value, description = p_description,
           modified_at_utc = now(), version = s.version + 1
     WHERE s.scope_code = v_scope_code AND s.scope_id IS NOT DISTINCT FROM v_scope_id AND s.name = p_name
    ...
    IF NOT FOUND THEN
        IF v_scope_id IS NULL THEN
            ...
            VALUES (v_scope_code, NULL, p_name, p_value_format_id, p_value, p_description, now(), now(), 0)
            ON CONFLICT (scope_code, name) WHERE scope_id IS NULL DO UPDATE
                SET value_format_id = EXCLUDED.value_format_id, value = EXCLUDED.value,
                    description = EXCLUDED.description, modified_at_utc = now(), version = s.version + 1
            RETURNING s.version INTO v_version;
        ELSE
            INSERT INTO {{schema}}.settings AS s
                (scope_code, scope_id, name, value_format_id, value, description, created_at_utc, modified_at_utc, version)
            VALUES (v_scope_code, v_scope_id, p_name, p_value_format_id, p_value, p_description, now(), now(), 0)
    ...
    VALUES (
        160 /* JobEventCode.SettingUpdated */, now(), COALESCE(v_namespace_id, 1), p_actor_code, p_actor_key,
        NULL, NULL, NULL, NULL, v_definition_id, NULL, NULL,
        NULL, NULL, NULL, NULL, NULL, p_reason_message,
        1 /* JobPayloadFormat.Json */, convert_to('{"name":"' || p_name || '"}', 'UTF8'));
