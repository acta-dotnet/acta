CREATE OR REPLACE FUNCTION {{schema}}.suspend_namespace(
...
BEGIN
    ...
    UPDATE {{schema}}.namespaces AS n
       SET status_code = 20 /* JobNamespaceStatusCode.Suspended */, modified_at_utc = now(), version = n.version + 1
     WHERE n.id = v_id
    ...
    VALUES (
        20 /* JobEventCode.NamespaceSuspended */, now(), v_id, p_actor_code, p_actor_key,
