...
BEGIN
    ...
    INSERT INTO _enq_batch (
        id, ordinal, job_ref, parent_id, lineage_root_id, deduplication_key, correlation_key,
        namespace_id, definition_id, tenant_id, input_format_id, input, exclusive_key,
        audit_level_code, priority_code, next_run_at_utc, is_child)
    SELECT
    ...
    WITH inserted_root AS (
        INSERT INTO {{schema}}.jobs (
            id, job_ref, lineage_root_id, parent_id,
        ...
        SELECT
            e.id, e.job_ref, NULL, NULL,
            e.deduplication_key, e.correlation_key,
            e.namespace_id, e.definition_id, e.tenant_id,
            e.input_format_id, e.input,
            e.exclusive_key, e.audit_level_code,
            now()
          FROM _enq_batch e
    ...
    inserted_child AS (
        ...
        OVERRIDING SYSTEM VALUE
        SELECT
            e.id, e.job_ref, e.lineage_root_id, e.parent_id,
            e.deduplication_key, e.correlation_key,
            e.namespace_id, e.definition_id, e.tenant_id,
            e.input_format_id, e.input,
            e.exclusive_key, e.audit_level_code,
    ...
    runtime_insert AS (
        ...
        SELECT
            e.id, e.namespace_id, 10 /* JobStatusCode.Ready */,
            e.priority_code, e.next_run_at_utc,
            0, 0, NULL,
            now(), 0
          FROM inserted i
        ...
        SELECT e.ordinal, j.id, j.job_ref
          ...
          INNER JOIN {{schema}}.jobs j
                  ...
                  OR (e.is_child
                      ...
                      AND j.deduplication_key = e.deduplication_key)
         WHERE e.deduplication_key IS NOT NULL
           AND NOT EXISTS (
               SELECT 1
                 FROM inserted i
                WHERE i.id = e.id)
