...
SELECT j.id, j.job_ref,
       ...
       w.deployment_version, w.status_code AS worker_status_code, w.last_seen_at_utc,
       le.reason_code, le.reason_message,
       lx.worker_id AS last_executed_worker_id, lxw.deployment_version AS last_executed_worker_name
  FROM {{schema}}.jobs j
  ...
  LEFT JOIN {{schema}}.events  le ON le.id =
      (SELECT MAX(e.id) FROM {{schema}}.events e WHERE e.job_id = j.id AND e.reason_code IS NOT NULL)
  LEFT JOIN {{schema}}.events  lx ON lx.id =
      (SELECT MAX(e.id) FROM {{schema}}.events e WHERE e.job_id = j.id AND e.worker_id IS NOT NULL)
