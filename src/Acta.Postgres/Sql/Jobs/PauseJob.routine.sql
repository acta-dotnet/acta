...
BEGIN
    ...
    IF v_audit_level = 20 /* JobAuditLevelCode.Audit */ THEN
        ...
        VALUES (
            71 /* JobEventCode.JobPaused */, now(), v_namespace_id,
            p_actor_code, p_actor_key,
            p_id, v_job_ref, v_execution_number,
            COALESCE(v_lineage_root_id, p_id), v_definition_id, v_tenant_id,
            NULL,
            v_from_status, 30 /* JobStatusCode.Paused */,
            NULL, NULL,
            p_reason_code, p_reason_message);
    END IF;

    RETURN QUERY SELECT 1 /* JobControlAction.Applied */::SMALLINT, 30 /* JobStatusCode.Paused */::SMALLINT;
END;
$$;
