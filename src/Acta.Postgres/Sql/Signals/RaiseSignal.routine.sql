...
BEGIN
    ...
    SELECT r.status_code, j.namespace_id, j.lineage_root_id, j.definition_id, j.tenant_id,
           r.execution_number, j.audit_level_code, j.job_ref
      INTO v_from_status, v_namespace_id, v_lineage_root_id, v_definition_id, v_tenant_id,
           v_execution_number, v_audit_level, v_job_ref
    ...
    IF v_from_status IN (100 /* JobStatusCode.Done */, 200 /* JobStatusCode.Failed */, 220 /* JobStatusCode.Cancelled */) THEN
    ...
    IF v_audit_level = 20 /* JobAuditLevelCode.Audit */ THEN
        ...
        VALUES (
            80 /* JobEventCode.JobSignalRaised */, v_now, v_namespace_id, p_actor_code, p_actor_key,
            p_job_id, v_job_ref, v_execution_number, COALESCE(v_lineage_root_id, p_job_id), v_definition_id, v_tenant_id, NULL,
            v_from_status, v_from_status, NULL, NULL, p_reason_code, p_reason_message);
    END IF;

    IF v_from_status = 20 /* JobStatusCode.Suspended */ THEN
        UPDATE {{schema}}.runtimes
           SET status_code = 10 /* JobStatusCode.Ready */, next_run_at_utc = v_now,
               modified_at_utc = v_now, version = version + 1
        ...
        IF v_audit_level = 20 /* JobAuditLevelCode.Audit */ THEN
            INSERT INTO {{schema}}.events (
                ...
                execution_status_code, duration_ms, reason_code, reason_message)
            VALUES (
                72 /* JobEventCode.JobResumed */, v_now, v_namespace_id, p_actor_code, p_actor_key,
                p_job_id, v_job_ref, v_execution_number, COALESCE(v_lineage_root_id, p_job_id), v_definition_id, v_tenant_id, NULL,
                20 /* JobStatusCode.Suspended */, 10 /* JobStatusCode.Ready */, NULL, NULL, p_reason_code, p_reason_message);
        END IF;
