...
CREATE UNIQUE INDEX IF NOT EXISTS ux_alerts_dedupe ON {{schema}}.alerts (namespace_id, deduplication_key, dedupe_window_start_utc) WHERE deduplication_key IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS {{schema}}.events (
    ...
    , CONSTRAINT ck_events_reason_code CHECK (reason_code IS NULL OR reason_code IN (10, 20, 21, 22, 23, 24, 30, 40, 41, 50, 51, 52, 53, 54, 60, 61, 62, 63, 100, 101))
...
CREATE INDEX IF NOT EXISTS ix_leases_reclaim_expired ON {{schema}}.leases (kind_code, expires_at_utc);
...
CREATE INDEX IF NOT EXISTS ix_runtimes_claim_ready ON {{schema}}.runtimes (namespace_id, priority_code DESC, next_run_at_utc, job_id) WHERE status_code = 10;
...
CREATE TABLE IF NOT EXISTS {{schema}}.schedules (
    ...
    next_run_at_utc timestamptz NULL,
    orphaned_at_utc timestamptz NULL,
    status_code smallint NOT NULL,
...
CREATE INDEX IF NOT EXISTS ix_schedules_namespace_next ON {{schema}}.schedules (namespace_id, next_run_at_utc, id) WHERE orphaned_at_utc IS NULL;
...
CREATE TABLE IF NOT EXISTS {{schema}}.steps (
    ...
    , CONSTRAINT ck_steps_reason_code CHECK (reason_code IS NULL OR reason_code IN (10, 20, 21, 22, 23, 24, 30, 40, 41, 50, 51, 52, 53, 54, 60, 61, 62, 63, 100, 101))
...
CREATE INDEX IF NOT EXISTS ix_workers_namespace_status_lastseen ON {{schema}}.workers (namespace_id, status_code, last_seen_at_utc);
CREATE INDEX IF NOT EXISTS ix_workers_status_lastseen ON {{schema}}.workers (status_code, last_seen_at_utc);
