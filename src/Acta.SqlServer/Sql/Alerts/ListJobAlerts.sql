SELECT TOP (@p_take)
       a.id, ns.name, a.job_id, a.origin_code, a.severity_code, a.kind_code,
       a.title, a.message, a.channel_name, a.occurrence_count, a.resolved_at_utc,
       a.delivery_status_code, a.retry_count, a.retry_after_utc, a.created_at_utc, a.modified_at_utc,
       a.job_ref, a.acknowledged_at_utc
 ...
 WHERE (@p_namespace_name IS NULL OR ns.name = @p_namespace_name)
   ...
   AND (@p_acknowledged IS NULL OR (CASE WHEN a.acknowledged_at_utc IS NOT NULL THEN 1 ELSE 0 END) = @p_acknowledged)
        ...
        SELECT 1
         ...
         WHERE NOT EXISTS (
               SELECT 1 FROM {{schema}}.tags tg
                WHERE tg.scope_code = 80 /* TagScopeCode.Alert */ AND tg.scope_id = a.id
                  AND tg.name = f.name
