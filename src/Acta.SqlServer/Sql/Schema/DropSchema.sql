...
SELECT @sql += N'DROP PROCEDURE {{schema}}.' + QUOTENAME(name) + N';' + CHAR(10) FROM sys.procedures WHERE SCHEMA_NAME(schema_id) = N'{{schema}}';
SELECT @sql += N'DROP FUNCTION {{schema}}.' + QUOTENAME(name) + N';' + CHAR(10) FROM sys.objects WHERE type IN ('FN','IF','TF') AND SCHEMA_NAME(schema_id) = N'{{schema}}';
...
SET @sql = N'';
SELECT @sql += N'DROP VIEW {{schema}}.' + QUOTENAME(name) + N';' + CHAR(10) FROM sys.views WHERE SCHEMA_NAME(schema_id) = N'{{schema}}';
SELECT @sql += N'DROP TABLE {{schema}}.' + QUOTENAME(name) + N';' + CHAR(10) FROM sys.tables WHERE SCHEMA_NAME(schema_id) = N'{{schema}}';
SELECT @sql += N'DROP SEQUENCE {{schema}}.' + QUOTENAME(name) + N';' + CHAR(10) FROM sys.sequences WHERE SCHEMA_NAME(schema_id) = N'{{schema}}';
...
SELECT @sql += N'DROP TYPE {{schema}}.' + QUOTENAME(name) + N';' + CHAR(10) FROM sys.types WHERE is_user_defined = 1 AND SCHEMA_NAME(schema_id) = N'{{schema}}';
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
