...
BEGIN
    ...
    BEGIN
        SELECT @namespace_id = n.id FROM {{schema}}.namespaces n WHERE n.name = @p_namespace_name;
        ...
        BEGIN
            SELECT @definition_id = d.id
              FROM {{schema}}.definitions d WHERE d.namespace_id = @namespace_id AND d.name = @p_job_name;
