...
BEGIN
    ...
    DECLARE @from_status TINYINT, @namespace_id SMALLINT, @lineage_root_id BIGINT,
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            COMMIT TRANSACTION;
            SELECT CAST(1 /* JobControlAction.Applied */ AS TINYINT) AS action, CAST(10 /* JobStatusCode.Ready */ AS TINYINT) AS status_code;
            RETURN;
        END

        COMMIT TRANSACTION;
        SELECT CAST(1 /* JobControlAction.Applied */ AS TINYINT) AS action, @from_status AS status_code;
    END TRY
    BEGIN CATCH
        IF XACT_STATE() <> 0
        BEGIN
