...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @from_status    = r.status_code,
        ...
        INSERT @schedule_ids SELECT id FROM {{schema}}.schedules WITH (UPDLOCK, ROWLOCK) WHERE job_id = @p_id;
        INSERT @alert_ids SELECT id FROM {{schema}}.alerts WITH (UPDLOCK, ROWLOCK) WHERE job_id = @p_id;
        INSERT @event_ids SELECT id FROM {{schema}}.events WITH (UPDLOCK, ROWLOCK) WHERE job_id = @p_id;

        DELETE FROM {{schema}}.tags
         WHERE (scope_code = 50 /* TagScopeCode.Job */ AND scope_id = @p_id)
        ...
        COMMIT TRANSACTION;
        SELECT CAST(1 /* JobControlAction.Applied */ AS TINYINT) AS action, CAST(NULL AS TINYINT) AS status_code;
    END TRY
    BEGIN CATCH
        IF XACT_STATE() <> 0
        BEGIN
