...
-- SQL Server variant: recursive CTE without the RECURSIVE keyword, and TOP for the child cap.
...
SELECT
    ...
    r.status_code,
    j.parent_id,
    pj.job_ref AS parent_job_ref,
    j.lineage_root_id,
    lr.job_ref AS lineage_root_job_ref,
    j.created_at_utc,
    r.modified_at_utc
FROM {{schema}}.jobs j
...
WITH anc (id, parent_id, depth) AS (
    SELECT
        a.id,
        a.parent_id,
        1
    ...
    WHERE
        a.id = (
            SELECT parent_id FROM {{schema}}.jobs
            WHERE id = @p_id
        )
    UNION ALL
    SELECT
