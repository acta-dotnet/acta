            ...
            BEGIN
                ...
                IF @recurring = 1
                    BEGIN
                        ...
                        -- The slot's next state is read from the schedules as they stand now, not from the
                        -- plan made before the handler ran; see IExecutionStore.CompleteExecutionAsync.
                        SET @slot_wake = (
                        ...
                        SET @to_status = CASE WHEN @slot_wake IS NULL THEN 30 /* JobStatusCode.Paused */ ELSE 10 /* JobStatusCode.Ready */ END;

                        UPDATE {{schema}}.runtimes
                        SET
                            status_code = @to_status,
                            next_run_at_utc = @slot_wake
                        WHERE job_id = @p_id;

                        -- The result row reports the derived state, which is what the caller uses to decide
                        -- whether to wake for the next run.
                        SET @final_status = @to_status;
                        SET @final_next_run = @slot_wake;
                ...
                IF
                    ...
                    BEGIN
                        ...
                        VALUES (
                            CASE
                                WHEN @to_status = 10 /* JobStatusCode.Ready */ THEN 50 /* EventCode.JobRecurringRolledOver */
