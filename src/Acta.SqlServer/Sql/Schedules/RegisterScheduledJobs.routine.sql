...
BEGIN
    ...
    BEGIN TRY
        ...
        UPDATE j
           SET input_format_id  = d.input_format_id,
               input            = d.input,
        ...
        UPDATE tgt
          ...
          INNER JOIN @p_schedules AS src
                  ON src.definition_id = sl.definition_id
        ...
        SELECT js.id
         ...
         WHERE js.orphaned_at_utc IS NULL
           AND NOT EXISTS (
               SELECT 1
                 FROM @p_schedules AS src
                WHERE src.definition_id = sl.definition_id
        ...
        SELECT definition_id, slot_id FROM @slots;
    END TRY
    BEGIN CATCH
        IF XACT_STATE() <> 0
        BEGIN
