...
BEGIN
    ...
    BEGIN TRY
        ...
        UPDATE r
           SET status_code          = CASE WHEN @p_start_executing = 1 THEN 50 /* JobStatusCode.Executing */ ELSE 40 /* JobStatusCode.Dispatched */ END,
               execution_number     = r.execution_number + 1,
               leased_by_worker_id  = @p_leased_by_worker_id,
               lease_expires_at_utc = @lease_expires,
               modified_at_utc      = @now,
               version              = r.version + 1
        OUTPUT inserted.job_id, j.job_ref, j.namespace_id, j.lineage_root_id, j.definition_id, j.tenant_id,
        ...
        BEGIN
            ...
            SELECT
                40 /* JobEventCode.JobExecutionStarted */, @now, c.namespace_id, 70 /* JobActorCode.Worker */, NULL,
                c.id, c.job_ref, c.execution_number,
                COALESCE(c.lineage_root_id, c.id), c.definition_id, c.tenant_id, @p_leased_by_worker_id,
    ...
    SELECT
        id, namespace_id, definition_id, execution_number,
        deduplication_key, correlation_key, exclusive_key,
        input_format_id, input, next_run_at_utc,
        @lease_expires AS lease_expires_at_utc,
        created_at_utc, failure_count, version, job_ref, tenant_id,
