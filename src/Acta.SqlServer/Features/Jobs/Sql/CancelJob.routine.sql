...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            COMMIT TRANSACTION;
            SELECT CAST(2 /* JobControlAction.NotFound */ AS TINYINT) AS action, CAST(NULL AS TINYINT) AS status_code, CAST(NULL AS BIGINT) AS parent_id;
            RETURN;
        END;
        ...
        BEGIN
            COMMIT TRANSACTION;
            SELECT CAST(3 /* JobControlAction.Rejected */ AS TINYINT) AS action, @from_status AS status_code, @parent_id AS parent_id;
            RETURN;
        END;
        ...
        COMMIT TRANSACTION;
        SELECT CAST(1 /* JobControlAction.Applied */ AS TINYINT) AS action, CAST(220 /* JobStatusCode.Cancelled */ AS TINYINT) AS status_code, @parent_id AS parent_id;
    END TRY
    BEGIN CATCH
        IF XACT_STATE() <> 0
        BEGIN
