DROP TABLE IF EXISTS temp._suspend_namespace;

CREATE TEMP TABLE _suspend_namespace AS
...
SELECT
    20 /* JobEventCode.NamespaceSuspended */, {{now}}, s.id, @p_actor_code, @p_actor_key,
...
FROM (SELECT 1) one
LEFT JOIN temp._suspend_namespace s ON 1 = 1;
