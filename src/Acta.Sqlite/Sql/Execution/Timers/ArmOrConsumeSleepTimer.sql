...
WHERE
    jt.job_id = @p_job_id AND jt.kind_code = 30 /* JobCheckpointKindCode.Timer */ AND jt.name = @p_name
...
SET
    ...
    version = version + 1
WHERE
    job_id = @p_job_id AND kind_code = 30 /* JobCheckpointKindCode.Timer */ AND name = @p_name
...
WHERE
    ...
    AND NOT EXISTS (
        SELECT 1
        FROM {{schema}}.checkpoints jt2
        WHERE jt2.job_id = @p_job_id AND jt2.kind_code = 30 /* JobCheckpointKindCode.Timer */ AND jt2.status_code = 10 /* JobCheckpointStatusCode.Pending */);

SELECT
    CASE
        WHEN EXISTS (SELECT 1 FROM temp._due_timer) THEN 2 /* SleepOutcome.Continue */
        WHEN jt.status_code = 10 /* JobCheckpointStatusCode.Pending */ THEN 1 /* SleepOutcome.Suspend */
        WHEN jt.status_code IS NOT NULL THEN 2 /* SleepOutcome.Continue */
        WHEN COALESCE(@p_resume_at_utc, {{now}} + (@p_delay_seconds) * 1000) > {{now}}
            AND EXISTS (SELECT 1 FROM {{schema}}.checkpoints x WHERE x.job_id = @p_job_id AND x.kind_code = 30 /* JobCheckpointKindCode.Timer */ AND x.status_code = 10 /* JobCheckpointStatusCode.Pending */) THEN 3 /* SleepOutcome.Reject */
        ELSE 2 /* SleepOutcome.Continue */
    END AS outcome_code,
    CASE
        WHEN jt.status_code = 10 /* JobCheckpointStatusCode.Pending */ THEN jt.due_at_utc
        ELSE NULL
    END AS due_at_utc
