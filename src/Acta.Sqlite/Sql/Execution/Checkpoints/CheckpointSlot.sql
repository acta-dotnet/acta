...
WHERE @p_action IN (10 /* CheckpointSlotAction.Set */, 30 /* CheckpointSlotAction.GetOrSet */)
...
SELECT @p_job_id, @p_kind_code, @p_name, @p_value_format_id, @p_value
...
DELETE FROM {{schema}}.checkpoints
 WHERE @p_action = 50 /* CheckpointSlotAction.Delete */
   AND job_id = @p_job_id
...
SELECT CASE
         ...
         WHEN @p_action = 40 /* CheckpointSlotAction.Exists */ THEN (
             CASE WHEN EXISTS (
                 SELECT 1
                   FROM {{schema}}.checkpoints AS ev
                  WHERE ev.job_id = @p_job_id
  ...
  LEFT JOIN {{schema}}.checkpoints AS jv
    ON @p_action IN (20 /* CheckpointSlotAction.Get */, 30 /* CheckpointSlotAction.GetOrSet */)
