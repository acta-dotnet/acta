...
SELECT
    ...
    p.audit_level_code, p.parent_id, p.job_ref, p.sig_state
FROM _ce_pre p
WHERE p.cur_status = 50 /* JobStatusCode.Executing */
  AND p.cur_worker = @p_leased_by_worker_id
  AND p.execution_number = @p_execution_number;

UPDATE {{schema}}.runtimes
   SET status_code          = (SELECT to_status FROM _ce_done),
...
SELECT
    102 /* JobEventCode.SchedulePauseExpired */, {{now}}, d.namespace_id,
    10 /* JobActorCode.Sys */, NULL,
    @p_id, d.job_ref, d.execution_number,
    COALESCE(d.lineage_root_id, @p_id), d.definition_id, d.tenant_id,
    NULL,
    NULL, NULL,
    NULL, NULL,
    NULL, js.name
...
INSERT INTO {{schema}}.events (
    ...
    reason_code, reason_message)
SELECT
    41 /* JobEventCode.JobExecutionFinished */, {{now}}, d.namespace_id,
    70 /* JobActorCode.Worker */, NULL,
    @p_id, d.job_ref, d.execution_number,
    COALESCE(d.lineage_root_id, @p_id), d.definition_id, d.tenant_id,
    @p_leased_by_worker_id,
    50 /* JobStatusCode.Executing */, d.to_status,
    CASE
  ...
  FROM _ce_done d
 WHERE d.audit_level_code = 20 /* JobAuditLevelCode.Audit */
...
SELECT
    ...
    @p_reason_code, @p_reason_message
  FROM _ce_done d
 WHERE @p_reschedule_status_code IS NOT NULL
   AND d.audit_level_code = 20 /* JobAuditLevelCode.Audit */
...
SELECT
    ...
    @p_reason_code, @p_reason_message
  FROM _ce_done d
 WHERE @p_handler_status_code IS NOT NULL
   AND d.audit_level_code = 20 /* JobAuditLevelCode.Audit */
...
SELECT
    80 /* JobEventCode.JobSignalRaised */, {{now}}, p.parent_ns, 10 /* JobActorCode.Sys */, NULL,
    p.parent_id, p.parent_ref, p.parent_exec, COALESCE(p.parent_lineage, p.parent_id), p.parent_def, p.parent_tenant, NULL,
    p.parent_status, p.parent_status, NULL, NULL, @p_reason_code, @p_reason_message
  FROM _ce_parent p
...
INSERT INTO {{schema}}.events (
    ...
    execution_status_code, duration_ms, reason_code, reason_message)
SELECT
    72 /* JobEventCode.JobResumed */, {{now}}, p.parent_ns, 10 /* JobActorCode.Sys */, NULL,
    p.parent_id, p.parent_ref, p.parent_exec, COALESCE(p.parent_lineage, p.parent_id), p.parent_def, p.parent_tenant, NULL,
    20 /* JobStatusCode.Suspended */, 10 /* JobStatusCode.Ready */, NULL, NULL, 60 /* JobEventReasonCode.JobSignalReleased */, NULL
  FROM _ce_parent p
 WHERE p.parent_status = 20 /* JobStatusCode.Suspended */
