...
SELECT
    ...
    NULL,
    NULL,
    json_extract(d.value, '$.deduplication_key'),
    NULL,
    @p_namespace_id,
    json_extract(d.value, '$.definition_id'),
    json_extract(d.value, '$.input_format_id'),
    acta_blob(json_extract(d.value, '$.input')),
    NULL,
...
SELECT
    @p_namespace_id,
    sl.slot_id,
    json_extract(s.value, '$.definition_id'),
    json_extract(s.value, '$.name'),
    40 /* ScheduleOriginCode.Definition */,
    json_extract(s.value, '$.expression'),
    json_extract(s.value, '$.time_zone_id'),
    json_extract(s.value, '$.expression_kind_code'),
    json_extract(s.value, '$.misfire_strategy_code'),
    json_extract(s.value, '$.next_run_at_utc'),
    NULL,
    NULL,
    10 /* ScheduleStatusCode.Active */,
    NULL,
    json_extract(s.value, '$.description')
...
SET
    ...
    version = version + 1
WHERE
    job_id IN (SELECT slot_id FROM temp._reg_slots)
    ...
    AND NOT EXISTS (
        ...
        JOIN temp._reg_slots sl ON sl.definition_id = json_extract(s.value, '$.definition_id')
        WHERE
            sl.slot_id = {{schema}}.schedules.job_id
