DROP TABLE IF EXISTS temp._ps_target;

CREATE TEMP TABLE _ps_target AS
...
SELECT
    100 /* JobEventCode.SchedulePaused */, {{now}}, j.namespace_id,
...
WHERE j.id = @p_job_id
  ...
  AND EXISTS (SELECT 1 FROM temp._ps_target);

UPDATE {{schema}}.schedules
   SET status_code      = 30 /* ScheduleStatusCode.Paused */,
...
SELECT
    CASE WHEN t.schedule_id IS NULL
...
LEFT JOIN temp._ps_target t ON 1 = 1
