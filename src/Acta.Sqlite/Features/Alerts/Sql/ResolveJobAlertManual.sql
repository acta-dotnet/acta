DROP TABLE IF EXISTS temp._resolve_job_alert;
...
SELECT
    141 /* JobEventCode.AlertResolved */, t.now_utc, t.namespace_id,
...
FROM temp._resolve_job_alert t
LEFT JOIN {{schema}}.jobs j ON j.id = t.job_id
LEFT JOIN {{schema}}.runtimes r ON r.job_id = t.job_id
WHERE t.already_resolved = 0;

UPDATE {{schema}}.alerts
   SET resolved_at_utc = (SELECT now_utc FROM temp._resolve_job_alert),
...
SELECT
    ...
    CASE WHEN t.id IS NULL THEN NULL
         ...
         ELSE t.now_utc END AS resolved_at_utc
