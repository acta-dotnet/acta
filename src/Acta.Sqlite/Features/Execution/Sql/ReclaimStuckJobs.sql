...
SELECT
    41 /* JobEventCode.JobExecutionFinished */, j.namespace_id,
    ...
    21 /* JobEventReasonCode.JobLeaseExpired */, 'Worker lease expired; reclaimed by the sys.recovery system job.'
  FROM {{schema}}.jobs j
  JOIN {{schema}}.runtimes r ON r.job_id = j.id
  INNER JOIN {{schema}}.definitions jd ON jd.id = j.definition_id
 WHERE j.id IN (SELECT id FROM temp._reclaim_jobs)
   AND j.audit_level_code IN (10 /* JobAuditLevelCode.Failures */, 20 /* JobAuditLevelCode.Audit */);

UPDATE {{schema}}.runtimes
   SET status_code          = CASE WHEN (runtimes.failure_count + 1) >= jd.max_attempts_effective
 ...
 WHERE j.id = runtimes.job_id
   AND runtimes.job_id IN (SELECT id FROM temp._reclaim_jobs)
RETURNING runtimes.job_id AS id, runtimes.status_code,
