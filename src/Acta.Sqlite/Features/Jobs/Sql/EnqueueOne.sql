...
SELECT
    @p_job_ref,
    COALESCE(pj.lineage_root_id, pj.id), @p_parent_id,
    @p_deduplication_key,
...
WHERE j.job_ref = @p_job_ref
  AND NOT EXISTS (SELECT 1 FROM {{schema}}.runtimes x WHERE x.job_id = j.id);
...
LEFT JOIN {{schema}}.jobs ex ON @p_deduplication_key IS NOT NULL
    AND @p_parent_id IS NULL
    AND ex.parent_id IS NULL
    AND ex.deduplication_key = @p_deduplication_key
    AND ex.namespace_id = (SELECT ns.id FROM {{schema}}.namespaces ns WHERE ns.name = @p_namespace_name)
LEFT JOIN {{schema}}.jobs exc ON @p_deduplication_key IS NOT NULL
    AND @p_parent_id IS NOT NULL
