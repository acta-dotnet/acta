DROP TABLE IF EXISTS temp._sso_target;

CREATE TEMP TABLE _sso_target AS
SELECT js.id AS schedule_id,
       CASE WHEN js.version <> @p_expected_version
...
WHERE j.id = @p_job_id
  ...
  AND EXISTS (SELECT 1 FROM temp._sso_target WHERE action = 1 /* JobControlAction.Applied */);

UPDATE {{schema}}.schedules
   SET expression_override   = @p_expression,
...
LEFT JOIN temp._sso_target t ON 1 = 1
