 ...
 WHERE @p_mutation = 2 /* TagMutationKind.Upsert */
   ...
   AND EXISTS (
       SELECT 1
        ...
        WHERE NOT EXISTS (
            SELECT 1 FROM {{schema}}.tags t, temp._tag_target x
   ...
   AND (SELECT COUNT(*)
         ...
         WHERE t.scope_code = @p_scope_code AND t.scope_id = x.scope_id) >= 32;
...
SELECT @p_scope_code, x.scope_id, x.namespace_id,
...
DELETE FROM {{schema}}.tags
 WHERE @p_mutation = 3 /* TagMutationKind.Remove */
   AND scope_code = @p_scope_code
   AND scope_id = (SELECT scope_id FROM temp._tag_target)
   AND name IN (SELECT json_extract(value, '$.name') FROM json_each(@p_items_json));

SELECT CASE WHEN EXISTS (SELECT 1 FROM temp._tag_target)
            THEN 1 /* TagMutationResult.Applied */
            ELSE 2 /* TagMutationResult.NotFound */ END AS action;
