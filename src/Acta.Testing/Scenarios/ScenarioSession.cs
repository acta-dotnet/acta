    ...
    where TInput : notnull
{
    ...
    public Task RunUntilDoneAsync(int maxTicks = DefaultMaxTicks, CancellationToken ct = default) =>
        RunUntilStatusAsync(JobStatusCode.Done, "reach Done", maxTicks, ct);
    ...
    {
        var page = await Host.Operations.ListJobEventsAsync(new ListJobEventsQuery(JobId: JobId, PageSize: 100), ct);
