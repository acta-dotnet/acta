...
/// <c>Note</c>) is a presence-only request. The framework stamps the actor itself, so the body carries
/// only the operator note.
/// </summary>
internal sealed record AlertControlRequest(string? Note = null);
